...
def change_alpha(master, x, y):
    ...
    if not have_alpha_flag:
        ...
        window_choose.bind('<B1-Motion>', move_alpha)
        window_choose.bind('<Button-1>', get_mouse)


def settings(master, x, y):
    def func(*args):
        global window_set, open_set, frame_bin_open_setting, frame_bin_window_setting, \
            frame_bin_color_setting, color_set
    ...
    combobox_choose['value'] = ('大小设置', '开机选项', '窗口颜色')
    combobox_choose.current(0)

    combobox_choose.bind('<<ComboboxSelected>>', func)
    window_settings.bind('<B1-Motion>', move_setting)
    window_settings.bind('<Button-1>', get_mouse)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        while events_can_happen and bin_name is not None:
            while bin_name is not None and count_sec < 300:
                count_sec += 1
                time.sleep(1)
            if count_sec != 300:
                break
